...
export default function Login() {
  ...
  const handleSubmit = async e => {
    ...
    if (step === 0) {
      // password step
      try {
        const res = await login(form.identifier.trim(), form.p);
        if (res.qrData) setQr(res.qrData);
        setStep(1);
      } catch {
      ...
      // TOTP step
      try {
        await verifyMfa(form.identifier.trim(), form.code.replace(/\s+/g,''));
        navigate('/dashboard');
      } catch {
